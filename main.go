...
import (
	...
	econstants "elasticai.io/elastic-queue/pkg/constants"
	"elasticai.io/elastic-queue/pkg/controller/workload/tfjob"
...
func setupIndexes(mgr ctrl.Manager) {
	...
	if err := tfjob.SetupIndexes(mgr.GetFieldIndexer()); err != nil {
		setupLog.Error(err, "Unable to setup tfjob indexes")
	}
}
...
func setupControllers(mgr ctrl.Manager, cCache *cache.Cache, queues *queue.Manager, certsReady chan struct{}, manageJobsWithoutQueueName bool) {
	...
	if err := tfjob.NewReconciler(mgr.GetScheme(),
		mgr.GetClient(),
		mgr.GetEventRecorderFor(econstants.TFJobControllerName),
		tfjob.WithManageJobsWithoutQueueName(manageJobsWithoutQueueName),
	).SetupWithManager(mgr); err != nil {
